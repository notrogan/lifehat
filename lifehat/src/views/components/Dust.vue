...
let dust = ref(0);
let dustProgress = ref(0);

onMounted(() => {
  setInterval(getDust, 1000);
})

async function getDust() : Promise<Record<string, any>[]> {
  const data = await dataService.getData("METRIC");

  dust.value = Math.round(data[0].dust);
  dustProgress.value = Math.max(300, Math.min(730 - Math.round(data[0].dust * 2), 735));
...
circle {
  ...
  stroke-dashoffset: v-bind(dustProgress);
}
